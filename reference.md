...
### 红冲 / 准发类查询页（材料信息 + 左下 F2 查询）

与「仓库图形化」不同布局的一类页：**左上「查询条件」**、**右上「查询结果」**、中间 **「材料信息」** 大表；表空时中央为 **「无相关数据」**，底栏 **总条数: 0**。

**必填规则（重要）**

- **库区号**为 **带红色 `*` 的下拉**，未选择有效库区前点 **「F2查询」**（如 `button[id*='authButton-F2']`，左下角 **`div.left-auth-button-box`** 内）通常仍无数据；Agent 应 **先展开库区号 → 选一项 → 再 snapshot → 再点 F2 查询**（其它条件可按用户口述选填）。

**查询条件区（左上）**

| 字段 | 控件 | 说明 |
|------|------|------|
| 库区号 | 下拉 * | 必选；先点击展开再选选项文案 |
| 材料号 | 文本 | 可选 |
| 准发计划号 | 文本 | 可选 |
| 提单号 | 文本 | 可选 |
| 合同号 | 文本 | 可选 |
...
- 表头含：材料号、厂别区分、准发计划号、准发单据号、提货单号(计划号)、合同号、红冲标记、库号、准发计划状态、物料种类、车船号、制造命令等（以环境为准）。
- 左下角 **F2查询** 与 `auto.py` 中 `_click_f2_query` 选择器一致。
